...
public class InitializerProcessor extends AbstractProcessor {
        ...
        for (Element element : roundEnv.getElementsAnnotatedWith(GenerateInitializer.class)) {
            ...
            if (!hasInit) {
                processingEnv.getMessager().printMessage(
                        ...
                        "Class '" + typeElement.getQualifiedName() + "' is annotated with @GenerateInitializer, but missing 'public void init()' method!",
                        typeElement
                );
            }

            if (isFabric && loaderAllowsFabric) {
                generateFabricInitializer(filer, mainClass, isClient, endPackage, annotation.addClassNameFabric(), hasInit);
            }

            if ((isForge || isNeoForge) && loaderAllowsForge) {
                generateForgeInitializer(filer, mainClass, modId, isClient, endPackage, annotation.addClassNameForge(), isNeoForge, hasInit);
            }
        }
        return true;
    }
    ...
    private void generateForgeInitializer(Filer filer, TypeName mainClass, String modId, boolean isClient,
                                          String packageId, String addClassName, boolean isNeoForge, boolean hasInit) {
        ...
        if (hasInit) {
            ...
            constructorBuilder.addStatement("$T.get().getModEventBus().addListener(this::onSetup)", contextClass);
    ...
    private void generateFabricInitializer(Filer filer, TypeName mainClass, boolean isClient,
                                           String packageId, String addClassName, boolean hasInit) {
    ...
    private boolean hasInitMethod(TypeElement typeElement) {
        for (Element enclosed : typeElement.getEnclosedElements()) {
            if (enclosed.getKind() == javax.lang.model.element.ElementKind.METHOD
                    && enclosed.getSimpleName().contentEquals("init")) {

                javax.lang.model.element.ExecutableElement method = (javax.lang.model.element.ExecutableElement) enclosed;
                if (method.getParameters().isEmpty() && method.getModifiers().contains(Modifier.PUBLIC)) {
                    return true;
                }
            }
        }
